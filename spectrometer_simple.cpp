                                       ...
                                       Controller* pController )
{
  ...
  m_accum1.init( m_uNumChannels, m_dStartFreq, 
                 m_dStopFreq, m_dChannelFactor );
  m_accum2.init( m_uNumChannels, m_dStartFreq, 
                 m_dStopFreq, m_dChannelFactor );
...
{ 
  ...
  while (!isStop(uCycle, totalRunTimer)) {
    ...
    for (unsigned int i=0; i<3; i++) {
      ...
      switch(i) {
        case 0:
          m_pCurrentAccum = &m_accum1;
          break;
        case 1:
          m_pCurrentAccum = &m_accum2;
          break;
      }

      // Mark state of new cycle
      tk.setNow();
      printf("Spectrometer: Starting accumulation %d at %s\n", i, tk.getDateTimeString(5).c_str());
...
// ----------------------------------------------------------------------------
void SpectrometerSimple::allocateAccumulators(unsigned int uNumItems)
{  
  ...
  Accumulator* pAccum = NULL;
	...
	for (unsigned int i=0; i<uNumItems; i++) {
		pAccum = new Accumulator();
		
		if (pAccum != NULL) {
			m_empty.push_front(pAccum);
		} else {
			printf("SpectrometerSimple::Allocate -- Failed to allocate accumulator %d of %d\n", i, uNumItems);
		}
	}	
